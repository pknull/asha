...
**Hooks: JSON for Claude, TOML for Codex, none for Copilot.** Claude reads `settings.json` (entries tagged `"source":"asha:<ns>"` for clean removal); Codex reads `config.toml` (a fenced `# asha:start … # asha:end` region) and doesn't support the `SessionEnd`/`Setup` events, which are dropped with a warning. Copilot's hook payload delivery was unreliable — and since `/save` now reads each harness's native session transcript directly (see Capture pipeline below), hooks are no longer needed for capture, so the Copilot hook install is retired. **Caveat (verified 2026-06-17):** asha writes its PreToolUse hooks into Codex's `config.toml`, but Codex does not appear to *fire* them for shell tool calls — both the policy guardrail and `block-secrets` are **installed-but-inert under Codex** (matcher tool-names and/or hook-trust). Treat **Claude as the enforced harness**; Codex hook enforcement is a known gap.
...
Cross-harness: enforcement is **Claude-only today**. The hook is also installed into Codex's `config.toml`, but Codex does **not** currently fire PreToolUse hooks for shell tool calls (verified 2026-06-17, interactive and `codex exec` — the broad-scan rule and `block-secrets` both fail to trigger under `asha codex`). Likely Claude-style tool-name matchers and/or Codex's hook-trust/coverage model; tracked as a **known gap**. The Codex deny-degradation logic is in place for when that's fixed. Copilot has no hook seam. See **[docs/harness-enforcement.md](docs/harness-enforcement.md)** for the full per-harness enforcement matrix, live-test findings, and what is / isn't fixable. The hook is **fail-open** — any rule/parse error allows the call, because a guardrail must never brick tool use. It is a **soft deterrent, not a sandbox**: it matches the command string by regex, so an agent can evade it deliberately (`cd /home && find .`, long flags, indirection). Pair it with the harness's own permission/sandbox controls for hard containment. This is the enforced form of the "Failure-to-Guardrail" idea: a high-confidence learning becomes a rule instead of prose a model can skip past.
