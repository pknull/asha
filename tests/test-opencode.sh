...
if install_into "$H1" --with-canary >"$WORK/install.out" 2>"$WORK/install.err"; then ok "install exits 0"; else fail "install exits 0"; fi
...
assert "valid canonical rendering emits no skipped-agent warning" '! grep -q "invalid OpenCode agent name.*character" "$WORK/install.err"'
...
if HOME="$H1" XDG_CONFIG_HOME="$H1/config" ASHA_HOME="$H1/.asha" \
    ...
    bash "$REPO_ROOT/bin/asha-drift-check.sh" --target opencode --with-canary \
...
assert "OpenCode read-only audit preserves drifted generated bytes" \
  '[[ $(sha256sum "$OC1/commands/session-save.md" | awk '\''{print $1}'\'') == "$stale_hash" ]]'
if install_into "$H1" --with-canary --force >/dev/null 2>&1; then
...
install_set() {
  HOME="$HSET" XDG_CONFIG_HOME="$HSET/config" ASHA_HOME="$HSET/.asha" \
    ASHA_OPENCODE_CMD="$OPENCODE_OK" \
    bash "$SET_REPO/install.sh" --target opencode --with-canary "$@"
}
drift_set() {
  HOME="$HSET" XDG_CONFIG_HOME="$HSET/config" ASHA_HOME="$HSET/.asha" \
    ASHA_OPENCODE_CMD="$OPENCODE_OK" \
    bash "$SET_REPO/bin/asha-drift-check.sh" --target opencode --with-canary
}
