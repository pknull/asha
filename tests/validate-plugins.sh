...
# Test 2c: A plugin skill's declared name is its portable destination name.
echo -n "Test 2c: Skill names equal <namespace>-<directory>... "
SKILL_NAME_ERRORS=()
while IFS= read -r skill_file; do
    [[ -f "$skill_file" ]] || continue
...
# Test 2d: Agent Skills permits only these six top-level frontmatter keys.
echo -n "Test 2d: Skill frontmatter uses only Agent Skills keys... "
SKILL_KEY_ERRORS=()
while IFS= read -r skill_file; do
    [[ -f "$skill_file" ]] || continue
...
# Test 2e: Prove both validators reject one deliberately broken fixture.
echo -n "Test 2e: Skill name and key validators fail closed... "
...
# Test 2f: Review discovery must follow the target path, not a legacy book root.
...
echo -n "Test 2f: Review paths are project-root relative... "
