...
| **Workspace v1 (detection, save scopes, commit gate — 2026‑08‑08)** | ✅ **full**: shared tools + PreToolUse `save-commit-gate` plane enforcement (Test 9b/9c live pins; Suite 15) | ⚠️ **detection + writer-proof verified** (live env-shaped probe 2026‑08‑08: `project_root.py workspace` resolved and the `save_scope` write-proof/verify round-trip passed under `CODEX_THREAD_ID` with no `CLAUDE_PROJECT_DIR`); gate registered in `~/.codex/config.toml` but **not enforced** — per the PreToolUse row above, codex shell interception did not fire; no auto-save lifecycle (pre-existing). Writer-side proof is the operative protection. | ⚠️ **detection + writer-proof verified** (same probe under `COPILOT_CLI=1`); auto-save lifecycle wired; **commit gate NOT chained** — `copilot-policy-adapter.sh` routes policy-guard only (pre-existing since the gate's creation; follow-up issue filed). Writer-side proof is the operative protection. |
