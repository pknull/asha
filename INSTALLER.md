...
| Hooks install | **Deferred** | Copilot CLI hooks are project-scope only (`.github/hooks/` or CWD). User-scope `~/.copilot/hooks/hooks.json` is not read. Set `ASHA_COPILOT_HOOKS_FORCE=1` to write anyway (use only if Copilot adds user-scope support in a future version) |
| Hook payload translator | Not built | Hook scripts assume Claude's `{tool_name, tool_input, tool_response}` JSON contract. Copilot's payload shape unverified; even with hooks installed, scripts may misbehave |
