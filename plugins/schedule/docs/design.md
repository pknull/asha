...
│  At scheduled time:                                             │
│  → Runs the per-task runner .claude/schedules.d/<id>.sh         │
│  → The runner execs: claude -p "command" --allowedTools ...     │
...
### 4. Per-Task Runner Scripts (generated)

There is **no** standalone `task-runner.sh`. `sync-schedules.py` generates one
small runner per enabled task at `.claude/schedules.d/<task-id>.sh`
(`write_task_runner`), and the systemd `ExecStart=` / crontab line references
only that controlled path. This is deliberate: every task-controlled value
(command, allowed tools, working directory) is embedded into the runner as a
shell-quoted token, so neither the unit text nor the crontab line ever
interpolates untrusted task content into a shell context — there is no
command-injection surface in the scheduler entries.

Each generated runner looks like:

```bash
#!/bin/bash
# Auto-generated by sync-schedules.py for task: <task-id>
# Regenerated on every `schedule sync` — do not edit by hand.
set -euo pipefail
cd '<workingDirectory>'
exec timeout <timeout> claude -p '<command>' \
    --allowedTools '<Read,Grep,Glob>' \
    >> '<project>/.claude/logs/<task-id>.log' 2>&1
```

Notes:

- The `timeout <n>` prefix is included only when a coreutils `timeout`
  (or `gtimeout`) is on `PATH`; otherwise the runner execs `claude` directly and
  relies on the systemd unit's `TimeoutStartSec` for a hard cap.
- Runners are written `0o700` and (re)written on every sync. Runners for tasks
  that are no longer enabled are pruned (`prune_task_runners`) on every sync,
  including the empty case — emptying `schedules.json` removes all of this
  project's runners, not just the scheduler entries.
- The runner streams stdout+stderr to `.claude/logs/<task-id>.log`. There is no
  separate status-file / `schedules.json` write-back step; execution history is
  the log.

### 5. Hook Integration

> Illustrative only (marketplace-era SessionStart injection). The shipped
> schedule plugin does not register this hook; `<plugin-root>` stands in for
> the installed hook directory under the symlink-mount install model.
...
```json
{
  "hooks": {
    "SessionStart": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "<plugin-root>/hooks/handlers/session-start.sh"
      }]
    }]
  }
}
