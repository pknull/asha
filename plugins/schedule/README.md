...
| `.claude/schedules.d/<task-id>.sh` | Per-task runner script (auto-generated, mode 0700) |
...
Each task gets its own runner script, generated by `sync-schedules.py` at
`.claude/schedules.d/<task-id>.sh` (mode 0700) and regenerated on every
`/schedule sync`. If a runner is missing or non-executable, re-run sync to
recreate it:
