#!/bin/bash
# Memory v2 SessionEnd: seal a recovery timestamp and prune expired snapshots.
# Semantic publication is exclusively the explicit /session:save command.
set -uo pipefail
...
if [[ -n "$SESSION_ID" && "$SESSION_ID" != unknown && -n "$PLUGIN_ROOT" && -n "$PYTHON_CMD" && -f "$PLUGIN_ROOT/tools/recovery_state.py" ]]; then
  "$PYTHON_CMD" "$PLUGIN_ROOT/tools/recovery_state.py" seal --project-dir "$PROJECT_DIR" \
    --harness "$HARNESS" --session-id "$SESSION_ID" >/dev/null 2>&1 || true
fi
echo '{}'
exit 0
