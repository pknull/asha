...
case "$TOOL" in
  ...
  read) TOOL_NAME="Read" ;;
  edit|patch|apply_patch) TOOL_NAME="Edit" ;;
...
CLAUDE_INPUT="$(jq -cn --arg tool "$TOOL_NAME" --arg sid "$SID" --arg cwd "$CWD" --argjson args "$ARGS" \
  '{tool_name: $tool, tool_input: $args, session_id: $sid, cwd: $cwd}')" || exit 0
